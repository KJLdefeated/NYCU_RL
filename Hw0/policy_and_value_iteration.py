...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for _ in range(max_iterations):
        value_ = value
        value = np.max(np.sum(P * (R + gamma * value), axis=-1), axis=-1)
        if np.abs(np.linalg.norm(value-value_, ord=np.inf)) < eps:
            break
    policy = np.argmax(np.sum(P * (R + gamma * value), axis=-1), axis=-1)
    #############################
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for _ in range(max_iterations):       
        while True:
            ...
            if np.linalg.norm(v-v_, ord=np.inf) < eps:
                break
        tmp_p = policy.copy()
        policy = np.argmax(np.sum(P * (R + gamma * v), axis=-1), axis = -1)
        if np.all(tmp_p == policy):
            break
    #############################
